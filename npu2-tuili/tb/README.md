...
**配置PE阵列大小**:
testbench默认配置为48x48的PE阵列。可以通过修改`tb/tb_npu_top.v`中的`ARRAY_SIZE`参数来改变阵列大小:

```verilog
// 在 tb/tb_npu_top.v 文件中修改
parameter ARRAY_SIZE = 48;     // 可选: 24, 48, 64, 96等
```

**常用配置及对应算力**:

| 配置 | PE阵列 | 总PE数 (4阵列) | INT8算力 @600MHz | BF16算力 @600MHz | 适用场景 |
|------|--------|---------------|-----------------|-----------------|---------|
| `ARRAY_SIZE = 24` | 24×24 | 2,304 | 2.76 TOPS | 2.76 TFLOPS | 快速仿真验证 |
| `ARRAY_SIZE = 48` | 48×48 | 9,216 | 11.06 TOPS | 11.06 TFLOPS | 中等规模调试(推荐) |
| `ARRAY_SIZE = 64` | 64×64 | 16,384 | 19.66 TOPS | 19.66 TFLOPS | 较大规模测试 |
| `ARRAY_SIZE = 96` | 96×96 | 36,864 | 44.24 TOPS | 44.24 TFLOPS | 完整规模(仿真慢) |

**算力计算方法**:
```
INT8/BF16算力 (TOPS/TFLOPS) = 4阵列 × PE数 × 2 ops/cycle × 600MHz ÷ 10^12
```

**注意**: 
- 实际有效算力约为理论峰值的40%-80%,取决于访存效率和流水线利用率
- 96×96配置对应原设计的满配算力(约22 TOPS INT8有效算力)

**注意**:
- 较小的阵列规模可以显著加速仿真,避免卡死
- 修改ARRAY_SIZE后需要重新编译: `make clean && make tb_npu_top`
- 阵列大小会影响计算周期数和性能计数器的值
